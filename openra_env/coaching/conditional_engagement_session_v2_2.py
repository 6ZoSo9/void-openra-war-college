"""Runtime-neutral session adapter for Conditional Engagement Candidate V2.2."""
...
COMMIT_SCHEMA = "void.apollyon.conditional-engagement-accepted-round.v2.2"
...
class ConditionalEngagementSessionV22:
    ...
    def prepare_round(self, state: Mapping[str, Any], *, round_no: int, allowed_tool_names: Sequence[str]) -> dict[str, Any]:
        ...
        alternatives = [name for name in names if name != "attack_move"] if decision["mode"] == "FORCE_CONVERSION" else []
        compliance = {
            "schema": "void.apollyon.conditional-engagement-action-compliance.v2.2",
            "mode": decision["mode"],
            "tool_surface_unchanged": True,
            "attack_move_discouraged": decision["mode"] == "FORCE_CONVERSION" and bool(alternatives),
            "preferred_offered_alternative": "move_units" if decision["mode"] == "FORCE_CONVERSION" and "move_units" in names else None,
        ...
        return {
            ...
            "coaching": format_coaching(decision, names),
