"""Candidate-only Git callbacks for the unchanged V2R13 materializer.

Construction requires confirmation of worktree operations, not runtime authority.
Only two fixed repository/commit/destination associations are accepted. There
is no CLI, discovery, fetch, checkout of the canonical repository, force removal,
arm-root deletion, or runtime call. Import and construction perform no host I/O.

The OS, /usr/bin/git, local repository configuration/attributes and cooperative
operator namespace remain trusted. This is not a sandbox for arbitrary repos or
hostile same-UID mutation. The outer invocation still owns durable attempt
consumption, current-main/source admission, readiness and runtime authorization.
"""
...
CONFIRM_TOKEN = "VOID_ABADDON_GENERATION2_V2R13_PREPARE_PAIR03_CANDIDATE_WORKTREES"
_SOURCE = "/home/zoso/dev/openra-rl-war-college"
_ARM = "/home/zoso/dev/void-war-college-execution/v2r13-generation2/generation2/pair-03/candidate"
_BINDINGS = (
    ...
    try:
        ...
        with selectors.DefaultSelector() as poller:
            ...
            while poller.get_map():
                ...
                _require(remaining > 0, "CANDIDATE_GIT_TIMEOUT")
                for key, _ in poller.select(remaining):
                    ...
                    if not chunk:
                        ...
                        continue
                    total += len(chunk)
                    _require(total <= MAX_OUTPUT_BYTES, "CANDIDATE_GIT_OUTPUT_BOUND")
        ...
        raise CandidateGitHold("CANDIDATE_GIT_TIMEOUT") from None
    except (OSError, UnicodeError):
        raise CandidateGitHold("CANDIDATE_GIT_TRANSPORT_HOLD") from None
    finally:
...
def _directory(path: str) -> os.stat_result | None:
    ...
    for part in Path(path).parts[1:]:
        ...
        try:
            ...
            return None
        _require(stat.S_ISDIR(last.st_mode), "CANDIDATE_GIT_PATH_NOT_REAL_DIRECTORY")
    return last


class CandidateGitBackend:
    ...
    def __init__(self, *, confirm: str):
        _require(type(confirm) is str and confirm == CONFIRM_TOKEN,
                 "CANDIDATE_GIT_CONFIRMATION_REQUIRED")
        _require(os.name == "posix", "CANDIDATE_GIT_PLATFORM_UNSUPPORTED")
        self._bindings = _BINDINGS
        self._created: dict[str, tuple[int, int]] = {}
        self._attempted: set[str] = set()

    def _binding(self, path: str) -> tuple[str, str, str]:
        _require(type(path) is str, "CANDIDATE_GIT_PATH_NOT_ALLOWED")
        for row in self._bindings:
            if path in row[:2]:
                return row
        raise CandidateGitHold("CANDIDATE_GIT_PATH_NOT_ALLOWED")
    ...
    def path_exists(self, path: str) -> bool:
        ...
        try:
            ...
            raise CandidateGitHold("CANDIDATE_GIT_PATH_OBSERVATION_HOLD") from None
    ...
    def run_git(self, repository_root: str, *args: str) -> subprocess.CompletedProcess[str]:
        ...
        _require(len(args) <= 5 and all(type(arg) is str for arg in args),
                 "CANDIDATE_GIT_COMMAND_NOT_ALLOWED")
        extra_reads = {
        ...
        _require(adding or removing or readable, "CANDIDATE_GIT_COMMAND_NOT_ALLOWED")
        try:
            _require(_directory(repository_root) is not None, "CANDIDATE_GIT_REPOSITORY_MISSING")
            if adding:
                _require(destination not in self._attempted, "CANDIDATE_GIT_ADD_ALREADY_ATTEMPTED")
                _require(_directory(str(Path(destination).parent)) is not None,
                         "CANDIDATE_GIT_PARENT_MISSING")
                _require(_directory(destination) is None, "CANDIDATE_GIT_DESTINATION_EXISTS")
            ...
            if removing:
                ...
                _require(observed is not None and destination in self._created
                         ...
                         "CANDIDATE_GIT_CLEANUP_NOT_OWNED")
            argv = [
            ...
            _require(result.returncode in allowed_codes, "CANDIDATE_GIT_COMMAND_FAILED")
            if adding:
                created = _directory(destination)
                _require(created is not None, "CANDIDATE_GIT_CREATED_PATH_MISSING")
                self._created[destination] = (created.st_dev, created.st_ino)
            if removing:
                _require(_directory(destination) is None, "CANDIDATE_GIT_CLEANUP_INCOMPLETE")
