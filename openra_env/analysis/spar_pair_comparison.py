...
class PairContractError(ValueError):
    """A supplied pair or matrix does not satisfy the comparison contract."""
...
def _clean_report(
    ...
    for key in (
        ...
        _require(
            ...
            f"{label} integrity not proven: {key}",
        )
...
def _tool_fraction(side: Mapping[str, Any], tool: str, rounds: int) -> float:
    ...
    _require(
        type(count) is int and 0 <= count <= rounds,
        f"tool count malformed: {tool}",
    )
    return count / rounds
...
def compare_reports(
    ...
    *,
    ...
    """Compare one exact baseline/V2 seed pair.

    Net kill-cost delta is the primary better/worse metric because that is the
    acceptance metric bound by the reviewed V2 gate. Force survival and action
    mix remain diagnostic and cannot silently override the primary verdict.
    """
    ...
    for field in PAIR_FIELDS:
        _require(
            bp.get(field) == cp.get(field),
            f"pair identity mismatch: {field}",
        )

    bv2 = _obj(
        baseline.get("conditional_engagement_v2"),
        "baseline.conditional_engagement_v2",
    )
    cv2 = _obj(
        candidate.get("conditional_engagement_v2"),
        "candidate.conditional_engagement_v2",
    )
    _require(
        bv2.get("present") is False,
        "baseline unexpectedly contains Conditional Engagement V2",
    )
    _require(
        cv2.get("present") is True,
        "candidate is missing Conditional Engagement V2",
    )
    _require(
        cv2.get("all_round_receipts_verified") is True,
        "candidate V2 round receipts are not verified",
    )
    _require(
        cv2.get("tool_surface_bound") is True,
        "candidate V2 tool surface is not bound",
    )
    _require(
        cv2.get("accepted_tool_bound") is True,
        "candidate V2 accepted tool is not bound",
    )
    _require(
        cv2.get("runtime_seed_branching") is False,
        "candidate V2 runtime seed branching detected",
    )
    _require(
        cv2.get("rounds_verified") == ci.get("rounds_completed"),
        "candidate V2 verified-round count mismatch",
    )

    if expected_v2_candidate_sha256 is not None:
        _require(
            cv2.get("candidate_sha256") == expected_v2_candidate_sha256,
            "unexpected V2 candidate SHA-256",
        )
    if expected_v2_source_commit is not None:
        _require(
            cv2.get("source_commit") == expected_v2_source_commit,
            "unexpected V2 source commit",
        )
    ...
    b_retries = list(
        _arr(b_ap["retried_rounds"], "baseline.apollyon.retried_rounds")
    )
    c_retries = list(
        _arr(c_ap["retried_rounds"], "candidate.apollyon.retried_rounds")
    )
    ...
    return {
        ...
        "baseline": {
            "trajectory_sha256": bp.get("trajectory_sha256"),
            "summary_sha256": bp.get("summary_sha256"),
            ...
            "apollyon_attack_move_fraction": _tool_fraction(
                b_ap, "attack_move", b_rounds
            ),
            "apollyon_attack_target_fraction": _tool_fraction(
                b_ap, "attack_target", b_rounds
            ),
            "apollyon_retried_rounds": b_retries,
        },
        "candidate": {
            "trajectory_sha256": cp.get("trajectory_sha256"),
            "summary_sha256": cp.get("summary_sha256"),
            "rounds_completed": c_rounds,
            "v2_source_commit": cv2.get("source_commit"),
            "v2_candidate_sha256": cv2.get("candidate_sha256"),
            "v2_mode_counts": dict(
                sorted(_obj(cv2.get("mode_counts"), "candidate V2 mode_counts").items())
            ),
            ...
            "apollyon_attack_move_fraction": _tool_fraction(
                c_ap, "attack_move", c_rounds
            ),
            "apollyon_attack_target_fraction": _tool_fraction(
                c_ap, "attack_target", c_rounds
            ),
        ...
        "comparison": {
            ...
            "final_combat_delta": (
                c_ap["final_combat_capable_units"]
                - b_ap["final_combat_capable_units"]
            ),
...
def evaluate_matrix(
    ...
    *,
    ...
    """Evaluate accumulated exact-pair evidence without promoting V2."""
    _require(
        type(regression_seed) is int and regression_seed >= 0,
        "regression_seed malformed",
    )
    _require(
        type(gain_seed) is int
        and gain_seed >= 0
        and gain_seed != regression_seed,
        "gain_seed malformed",
    )
    _require(
        type(minimum_held_out_seed_count) is int
        and minimum_held_out_seed_count >= 1,
        "held-out seed count malformed",
    )
    _require(
        type(repeats_per_seed) is int and repeats_per_seed >= 1,
        "repeats_per_seed malformed",
    )
    ...
    for index, pair in enumerate(pairs):
        ...
        _require(
            ...
            and pair.get("candidate_only") is True,
            f"pairs[{index}] identity drift",
        )
        ...
        _require(
            isinstance(candidate_sha, str) and bool(candidate_sha),
            "pair candidate SHA missing",
        )
        _require(
            isinstance(source_commit, str) and bool(source_commit),
            "pair source commit missing",
        )
        ...
        _require(
            ...
            "mixed V2 candidate SHA-256 generations",
        )
        _require(
            source_commit == expected_source_commit,
            "mixed V2 source commit generations",
        )
        all_protocol_clean = (
            all_protocol_clean and comparison.get("protocol_clean") is True
        )
    ...
    for seed, rows in sorted(grouped.items()):
        deltas = [
            _num(
                _obj(row["comparison"], "comparison").get("net_kill_cost_delta"),
                "net_kill_cost_delta",
            )
            for row in rows
        ]
        verdicts = [
            _obj(row["comparison"], "comparison").get("verdict") for row in rows
        ]
        _require(
            all(verdict in {"BETTER", "WORSE", "TIE"} for verdict in verdicts),
            f"seed {seed} verdict malformed",
        )
        complete = len(rows) == repeats_per_seed
        _require(
            len(rows) <= repeats_per_seed,
            f"seed {seed} exceeds repeat ceiling",
        )
        ...
        summary = {
            ...
            "all_protocol_clean": all(
                _obj(row["comparison"], "comparison").get("protocol_clean") is True
                for row in rows
            ),
        }
        if complete:
    ...
    held_out_complete = [
        ...
        if seed not in {regression_seed, gain_seed}
        and len(rows) == repeats_per_seed
    ]
    reviewed_complete = all(
        len(grouped.get(seed, [])) == repeats_per_seed
        for seed in (regression_seed, gain_seed)
    )
    complete = (
        reviewed_complete
        and len(held_out_complete) >= minimum_held_out_seed_count
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Fail-closed Conditional Engagement V2 pair/matrix comparison"
    )
    sub = parser.add_subparsers(dest="command", required=True)

    pair_parser = sub.add_parser("pair")
    pair_parser.add_argument("--baseline-report", required=True)
    pair_parser.add_argument("--candidate-report", required=True)
    pair_parser.add_argument("--expected-v2-candidate-sha256", required=True)
    pair_parser.add_argument("--expected-v2-source-commit", required=True)

    matrix_parser = sub.add_parser("matrix")
    matrix_parser.add_argument("--pair-report", action="append", required=True)
    matrix_parser.add_argument("--regression-seed", type=int, default=2051)
    matrix_parser.add_argument("--gain-seed", type=int, default=2055)
    matrix_parser.add_argument("--minimum-held-out-seed-count", type=int, default=3)
    matrix_parser.add_argument("--repeats-per-seed", type=int, default=3)
