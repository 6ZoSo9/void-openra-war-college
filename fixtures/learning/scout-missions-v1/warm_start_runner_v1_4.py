...
def parse_args():
    ...
    p.add_argument("--doctrine", default="RUSHER")
...
def apollyon_decision_typed(
    ...
    tools, contract = apollyon_tools_typed(base, state, pending)

    offered_names = list(contract["offered_tool_names"])
    offered = set(offered_names)

    # Do not mention any concrete production function that is not guaranteed
    # to exist on this exact turn. The current tool list is the sole action
    ...
    system = (
        ...
        "Call exactly one function from the CURRENT PROVIDED TOOL LIST. "
        "Do not infer, construct, remember, rename, or reuse a function that is absent from "
        "the current list. Production choices are encoded in the provided function identities. "
        "Use only observed own actor IDs and currently visible enemy IDs. Never invent IDs. "
        "Win the duel while preserving a functional economy: maintain power, production, scouting, "
        "contact response, tactical movement, retreat/counterattack when appropriate, and reinforcement. "
        "Host-controlled building placement is mechanical and symmetric. "
        "The advance tool is a deliberate no-command tactical choice, not an error fallback. "
    ...
    base_user = (
        ...
        f"CURRENT_ALLOWED_TOOL_NAMES={json.dumps(offered_names, separators=(',', ':'))}\n"
        f"STATE={json.dumps(base.compact_state(state), sort_keys=True, separators=(',', ':'))}"
    )
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if feedback:
            user += (
                ...
                + "\nThe world state is unchanged. Call exactly one name from "
        ...
        if not name:
            ...
            ok, reason, commands = decision_to_commands_typed(
                base,
                name,
                args,
                state,
        ...
        if ok:
            return name, (args or {}), commands, attempts, contract

        feedback = reason

    # Critical training-integrity rule: never manufacture an `advance` decision
    # after protocol-invalid model output. A completed trajectory must contain
    # an actual accepted Apollyon choice for every joint round.
    raise RuntimeError(
        "Apollyon failed to produce a valid current-turn tool call after "
        f"{max_attempts} attempts at round {round_no}; "
...
def main():
    ...
    print("=== VOID ACTUAL APOLLYON VS ABADDON WARM-START COMBAT SPAR V1.4 ===")
    ...
    print("controller_rows_training_candidate=true")
    ...
    print("invalid_tool_retry_without_world_mutation=true")
    print("invalid_tool_exhaustion_aborts_bout=true")
    ...
    if base.out(["git", "rev-parse", "HEAD"], cwd=RL) != WAR_COLLEGE_COMMIT:
        raise RuntimeError("War College not at frozen commit")
    ...
    base.exact_file(base.APOLLYON_RUNNER, base.APOLLYON_RUNNER_SHA, "Apollyon boundary runner")
    base.exact_file(base.ABADDON_CONTROLLER, base.ABADDON_CONTROLLER_SHA, "Abaddon controller")
    base.exact_file(JOINT_ATTESTATION, JOINT_ATTESTATION_SHA, "joint training attestation")
    print("frozen_training_boundary_green=true")

    helper = base.load_module(base.APOLLYON_RUNNER, "void_apollyon_v113_warmstart")
    if getattr(helper, "MODEL", None) != base.APOLLYON_MODEL:
        raise RuntimeError("Apollyon model boundary drift")
    for label_name, (path, expected) in helper.EXACT.items():
        base.exact_file(path, expected, label_name)
    base.exact_file(helper.V14, helper.V14_SHA, "Apollyon V14 boundary")
    print("apollyon_local_only_boundary_green=true")

    # Dormant preflight. Start Ollama only after warm-start handoff.
    active_cp = base.run(["systemctl", "is-active", helper.SERVICE], check=False, quiet=True)
    enabled_cp = base.run(["systemctl", "is-enabled", helper.SERVICE], check=False, quiet=True)
    ...
    if active != "inactive" or enabled != "disabled":
        raise RuntimeError("Ollama must begin inactive and disabled")
    ...
    run_id = (
        f"warmstart-apollyon-vs-abaddon-{stamp}-"
    ...
    with tempfile.TemporaryDirectory(prefix="void-warmstart-spar.") as td:
        ...
        try:
            ...
            # Start actual Apollyon only after scripted curriculum initialization.
            helper.start_ollama()
            ollama_started = True
            print("apollyon_runtime_started_at_handoff=true")
            ...
            append_jsonl(traj, {
                ...
                "apollyon_model": base.APOLLYON_MODEL,
            ...
            summary = {
                ...
                "winner": winner,
                "outcome": outcome,
                "apollyon_final": base.compact_state(states["Multi0"]),
            ...
            if ollama_started:
                helper.cleanup()
            print("warm_start_spar_cleanup_complete=true")
