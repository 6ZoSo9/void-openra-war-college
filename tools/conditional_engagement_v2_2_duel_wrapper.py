#!/usr/bin/env python3
"""Fail-closed designated-host wrapper for Conditional Engagement Candidate V2.2."""
...
OVERLAY_PACKAGE = "void_wc_conditional_v2_2_overlay"
...
def validate_v22_source(source_dir: Path) -> dict[str, str]:
    source_dir = source_dir.expanduser().resolve()
    _require(source_dir.is_dir(), f"V2.2 source dir missing: {source_dir}")
    head = parent._git(source_dir, "rev-parse", "HEAD")
    _require(
        head == V22_SOURCE_COMMIT,
        f"V2.2 source head drift: expected={V22_SOURCE_COMMIT} actual={head}",
    )
    dirty = parent._git(source_dir, "status", "--porcelain", "--untracked-files=no")
    _require(not dirty, "V2.2 source tracked worktree must be clean")

    fixture = source_dir / V22_FIXTURE
    policy = source_dir / "openra_env/coaching/conditional_engagement_v2_2.py"
    session = source_dir / "openra_env/coaching/conditional_engagement_session_v2_2.py"
    parent_policy = source_dir / "openra_env/coaching/conditional_engagement_v2_1.py"
    for label, path in (
        ("candidate fixture", fixture),
        ("policy module", policy),
        ("session module", session),
        ("parent V2.1 policy module", parent_policy),
    ):
        _require(path.is_file(), f"missing V2.2 {label}: {path}")

    fixture_sha = parent.sha256(fixture)
    _require(
        fixture_sha == V22_CANDIDATE_SHA256,
        f"V2.2 candidate fixture hash drift: expected={V22_CANDIDATE_SHA256} actual={fixture_sha}",
    )
    return {
        ...
        "parent_v2_1_policy_sha256": parent.sha256(parent_policy),
    }


def load_v22_overlay(source_dir: Path):
    source_dir = source_dir.expanduser().resolve()
    coaching_dir = source_dir / "openra_env" / "coaching"
    package_name = OVERLAY_PACKAGE
    _require(package_name not in sys.modules, "V2.2 overlay package already loaded")
    ...
    try:
        for basename in (
            ...
            "conditional_engagement_session_v2_2",
        ):
            name = f"{package_name}.{basename}"
            path = coaching_dir / f"{basename}.py"
            spec = importlib.util.spec_from_file_location(name, path)
            _require(spec is not None and spec.loader is not None, f"cannot load V2.2 module {path}")
            module = importlib.util.module_from_spec(spec)
            sys.modules[name] = module
            loaded.append(name)
            spec.loader.exec_module(module)
        return (
            sys.modules[f"{package_name}.conditional_engagement_v2_2"],
            sys.modules[f"{package_name}.conditional_engagement_session_v2_2"],
        )
    except Exception:
...
def load_candidate(policy_module: Any, source_dir: Path) -> Mapping[str, Any]:
    fixture = source_dir.expanduser().resolve() / V22_FIXTURE
    ...
    _require(actual == V22_CANDIDATE_SHA256, f"V2.2 candidate semantic digest drift: {actual}")
    return candidate


class ConditionalV22Hooks:
    ...
    def install(self) -> None:
        _require(not self._installed, "V2.2 hooks already installed")
    ...
    def _wrapped_decision(self, base, helper, state, pending, pb2, doctrine, round_no):
        ...
        _require(
            ...
            "offered tool surface changed between V2.2 preparation and host acceptance",
        )
    ...
    def _wrapped_append_jsonl(self, path: Path, row: dict[str, Any]):
        ...
        if event == "run_header":
            ...
            _require(
                ...
                f"missing V2.2 evidence for joint_decision round {round_no}",
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    source_dir = Path(wrapper_args.v2_2_source_dir)

    provenance = validate_v22_source(source_dir)
    policy_module, session_module = load_v22_overlay(source_dir)
    candidate = load_candidate(policy_module, source_dir)
    coaching_session = session_module.ConditionalEngagementSessionV22(candidate)
    ...
    _require(
        ...
        "V2.2 source must be a separate worktree; refusing frozen legacy worktree",
    )

    wrapper_sha = parent.sha256(SCRIPT_PATH)
    hooks = ConditionalV22Hooks(legacy, coaching_session, provenance, wrapper_sha)

    print("=== VOID CONDITIONAL ENGAGEMENT V2.2 DUEL WRAPPER ===")
    ...
    print(f"v2_2_policy_sha256={provenance['policy_sha256']}")
    print(f"v2_2_session_sha256={provenance['session_sha256']}")
    print(f"v2_2_wrapper_sha256={wrapper_sha}")
...
def cli(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print("VOID_CONDITIONAL_ENGAGEMENT_V2_2_WRAPPER_HOLD", file=sys.stderr)
