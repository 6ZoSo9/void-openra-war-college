...
def test_contract_closes_attempt_claim_and_invocation_implementation_only():
    out = invocation.pair06_v8_baseline_attempt_invocation_contract()
    assert out["pair06_v8_baseline_attempt_invocation_implemented"] is True
    assert out["pair06_v8_baseline_attempt_invocation_reviewed"] is False
...
def test_create_only_writer_is_durable_and_refuses_collision(tmp_path):
    ...
    with pytest.raises(
        invocation.Pair06V8BaselineAttemptInvocationHold,
...
def test_execution_entrypoint_holds():
    with pytest.raises(
        invocation.Pair06V8BaselineAttemptInvocationHold,
        match="PAIR06_V8_BASELINE_ATTEMPT_INVOCATION_SOURCE_BINDING_REVIEW_REQUIRED",
    ):
