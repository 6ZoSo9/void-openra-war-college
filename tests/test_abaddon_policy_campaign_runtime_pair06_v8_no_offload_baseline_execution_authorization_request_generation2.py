...
def test_request_is_proposal_only_and_non_authorizing():
    out = request.pair06_v8_no_offload_baseline_execution_authorization_request_contract()
    assert out["pair06_v8_no_offload_baseline_execution_authorization_request_implemented"] is True
...
def test_request_bytes_are_fixed_and_tampering_fails():
    payload = request.build_pair06_v8_no_offload_baseline_execution_authorization_request()
    result = request.validate_pair06_v8_no_offload_baseline_execution_authorization_request(
        payload
    )
    ...
    with pytest.raises(
        request.Pair06V8NoOffloadBaselineExecutionAuthorizationRequestHold,
        match="request bytes differ",
    ):
        request.validate_pair06_v8_no_offload_baseline_execution_authorization_request(
            tampered
        )
