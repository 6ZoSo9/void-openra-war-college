"""Evidence-preserving archival gate for the second consumed pair-06 V8 baseline attempt.
...
CONTRACT_SCHEMA = (
    ...
    "pair06-v8-second-failed-attempt-preservation-contract.v1"
)
RECEIPT_SCHEMA = (
    "void.abaddon.generation2."
    "pair06-v8-second-failed-attempt-preservation-receipt.v1"
)
...
PRIOR_ARCHIVE_ROOT = (
    ARCHIVE_PARENT / "baseline-20260923T221259Z-56c02591"
)
PRIOR_MARKER = (
    PRIOR_ARCHIVE_ROOT
    / "claims-v1"
...
CONFIRM_TOKEN = "VOID_PAIR06_V8_PRESERVE_SECOND_FAILED_ATTEMPT_446d8f92"

NEXT_GATE = "PAIR06_V8_SECOND_FAILED_ATTEMPT_PRESERVATION_SOURCE_BINDING_REVIEW_REQUIRED"
NEXT_CHANGE_CLASS = "source_only_pair06_v8_second_failed_attempt_preservation_review"


class Pair06V8SecondFailedAttemptPreservationHold(RuntimeError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise Pair06V8SecondFailedAttemptPreservationHold(message)
...
def _validate_prior_archive() -> dict[str, Any]:
    _require(
        PRIOR_ARCHIVE_ROOT.is_dir() and not PRIOR_ARCHIVE_ROOT.is_symlink(),
        "prior failed-attempt archive missing",
    )
    _require(
        _sha256_plain_file(PRIOR_MARKER) == PRIOR_MARKER_SHA256,
        "prior failed-attempt marker drift",
    )
    _require(
        _sha256_plain_file(PRIOR_PRESERVATION_RECEIPT)
        == PRIOR_PRESERVATION_RECEIPT_FILE_SHA256,
        "prior preservation receipt drift",
    )
    return {
        "archive_path": str(PRIOR_ARCHIVE_ROOT),
        "marker_sha256": PRIOR_MARKER_SHA256,
...
def _marker(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise Pair06V8SecondFailedAttemptPreservationHold(
...
def preserve_second_failed_pair06_v8_attempt(*, confirm: str) -> dict[str, Any]:
    _require(
        confirm == CONFIRM_TOKEN,
        "PAIR06_V8_SECOND_FAILED_ATTEMPT_PRESERVATION_CONFIRMATION_REQUIRED",
    )
    accepted = (
        forensics_acceptance
        .pair06_v8_second_failed_attempt_forensics_acceptance_contract()
    )
    _require(
        accepted.get("pair06_v8_second_failed_attempt_forensics_accepted") is True,
        "pair06 second failed-attempt forensics not accepted",
    )
    _require(
        accepted.get("attempt_marker_sha256") == ATTEMPT_MARKER_SHA256,
        "pair06 second failed-attempt marker identity drift",
    )
    _require(
    ...
    _require(not PRESERVATION_RECEIPT.exists(), "preservation receipt already exists")
    prior_archive = _validate_prior_archive()
    ...
    _require(
        _validate_prior_archive() == prior_archive,
        "prior failed-attempt archive changed during second preservation",
    )
    ...
    body = {
        ...
        "prior_failed_attempt_archive": prior_archive,
...
def pair06_v8_second_failed_attempt_preservation_contract() -> dict[str, Any]:
    accepted = (
        forensics_acceptance
        .pair06_v8_second_failed_attempt_forensics_acceptance_contract()
    )
...
def authorize_retry(*args: Any, **kwargs: Any) -> None:
    raise Pair06V8SecondFailedAttemptPreservationHold(
        "PAIR06_V8_SECOND_FAILED_ATTEMPT_RETRY_NOT_AUTHORIZED"
    )
