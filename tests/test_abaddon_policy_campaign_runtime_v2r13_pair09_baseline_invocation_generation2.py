"""Pair-09 baseline invocation wiring with inert runtime simulation."""
...
TREE = "b" * 40
SELF_SHA = "c" * 64
HOLD = invocation.V2R13Pair09BaselineInvocationHold


def invoke(**overrides):
    return invocation.execute_pair09_baseline(
        **{
...
def valid_receipt(root):
    out = {
        ...
        "run_artifact": {
            "run_id": "inert-pair09-baseline-fixture",
            "run_dir": str(
                root / "runs/inert-pair09-baseline-fixture"
            ),
...
def env(tmp_path, monkeypatch):
    ...
    preserved_rel = (
        Path("generation2/pair-03/preserved") / "evidence.json"
    )
    preserved = root / preserved_rel
    preserved.parent.mkdir(parents=True)
    preserved.write_bytes(b'{"pair03":"preserved"}\n')
    ...
    monkeypatch.setattr(
        invocation,
        "ARM_ROOT",
        root / "generation2/pair-09/baseline",
    )
    ...
    def collect(**kwargs):
        ...
        preserved_rows = invocation._preserved_pair03(preflight)
        result = {
            ...
            "pair03_preserved_evidence": preserved_rows,
        }
    ...
    def execute(**kwargs):
        ...
        marker = (
            root
            / invocation.CLAIMS_NAME
            / attempt.MARKER_NAME
        )
        assert marker.is_file()
        assert kwargs["pair_slot"] == 9
        assert kwargs["arm"] == "baseline"
        assert kwargs["candidate_genome_path"] is None
        assert kwargs["isolated_workdir_root"] == str(root)
        assert kwargs["execution_authorized"] is True
        assert (
            kwargs["run_git"].__self__
            is kwargs["path_exists"].__self__
        )
        assert isinstance(
            kwargs["run_git"].__self__,
            git.Pair09BaselineGitBackend,
        )
        assert kwargs["authority_check"](9, "baseline") is True
        assert kwargs["authority_check"](9, "candidate") is False
        ...
        try:
            ...
            if not kwargs["authority_check"](9, "baseline"):
                raise HOLD("inert-revoked-after-readiness")
        finally:
            events.append("runtime-cleanup-fixture")

        result = valid_receipt(invocation.ARM_ROOT)
    ...
    monkeypatch.setattr(
        preflight,
        "collect_pair09_baseline_host_preflight",
        collect,
    )
    return state
...
def test_contract_is_implemented_but_non_authorizing():
    out = invocation.pair09_baseline_invocation_contract()
    assert out["pair09_baseline_invocation_implemented"] is True
    assert out["pair09_baseline_invocation_reviewed"] is False
...
def test_one_shot_simulation_consumes_marker_before_executor_and_writes_result(
    env,
):
    out = invoke()
    ...
    assert out["pair09_baseline_execution_performed"] is True
...
def test_invalid_inputs_stop_before_components_or_host_access(
    monkeypatch,
    overrides,
):
    def forbidden():
...
def test_caller_cannot_supply_broad_callbacks_paths_or_pair_slot():
    for key in (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pair09_baseline_execution_authorized", True),
        ("observed_host_snapshot_sha256", "0" * 64),
        ("pair03_preserved_evidence", {}),
    ],
)
...
def test_postclaim_failures_preserve_marker_and_never_retry(env, failure):
    ...
    with pytest.raises(
        attempt.V2R13Pair09BaselineAttemptHold,
...
def test_wrong_executor_results_cannot_publish_success(
    env,
    field,
    value,
):
...
def test_pair03_evidence_change_after_executor_refuses_result(env):
...
def test_readiness_callback_rejects_wrong_context(env):
    def wrong(**kwargs):
        for key, value in (
            ("pair_slot", 3),
            ("pair_slot", True),
            ("arm", "candidate"),
            ("runtime_selection_key", "other"),
        ):
            context = {
                "pair_slot": 9,
                "arm": "baseline",
            ...
            context[key] = value
            with pytest.raises(HOLD, match="CONTEXT"):
                kwargs["readiness_provider"](context)
        raise HOLD("inert-test-complete")

    env.parts.executor.execute_v2r13_arm = wrong

    with pytest.raises(HOLD, match="test-complete"):
        invoke()

    assert "readiness" not in env.events


def test_interpreter_check_requires_designated_venv(monkeypatch):
    monkeypatch.setattr(
        invocation.sys,
        "executable",
        str(invocation.PROTO_PYTHON),
    )
    monkeypatch.setattr(
        invocation.sys,
        "prefix",
        str(invocation.PROTO_PYTHON.parent.parent),
    )
    monkeypatch.setattr(invocation.sys, "base_prefix", "/usr")
    monkeypatch.setattr(invocation.sys, "dont_write_bytecode", True)
    invocation._check_python()
