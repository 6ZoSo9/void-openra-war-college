"""Proposal-only request for one fresh no-offload pair-06 V8 baseline execution.

This request is distinct from both spent pair-06 execution requests and both
consumed attempts. It binds the reviewed no-offload invocation generation and
the successful preservation of both failed attempts. Matching these bytes never
grants authority.
...
REQUEST_SCHEMA = (
    ...
    "pair06-v8-no-offload-baseline-execution-authorization-request.v1"
)
VALIDATION_SCHEMA = (
    "void.abaddon.generation2."
    "pair06-v8-no-offload-baseline-execution-authorization-request-validation.v1"
)

INVOCATION_REVIEW_GIT_BLOB = "96c50cbf838c17aa4910ad9294fac345f30f41f7"
INVOCATION_REVIEW_SOURCE_SHA256 = (
    "d274cb8235fbf7e619bb75f1c0c4ce3ee91c1cee5306216bb033ee694bfe2fa5"
)
...
class Pair06V8NoOffloadBaselineExecutionAuthorizationRequestHold(ValueError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise Pair06V8NoOffloadBaselineExecutionAuthorizationRequestHold(message)
...
def build_pair06_v8_no_offload_baseline_execution_authorization_request() -> bytes:
    _dependencies()
    return (
        json.dumps(
            _request_record(),
...
def validate_pair06_v8_no_offload_baseline_execution_authorization_request(
    payload: bytes,
) -> dict[str, Any]:
    _require(type(payload) is bytes, "request must be exact bytes")
    _require(0 < len(payload) <= MAX_REQUEST_BYTES, "request byte limit")
    expected = build_pair06_v8_no_offload_baseline_execution_authorization_request()
...
def pair06_v8_no_offload_baseline_execution_authorization_request_contract() -> dict[str, Any]:
    result = validate_pair06_v8_no_offload_baseline_execution_authorization_request(
        build_pair06_v8_no_offload_baseline_execution_authorization_request()
    )
    return {
        **result,
        "pair06_v8_no_offload_baseline_execution_authorization_request_implemented": True,
...
def authorize_or_execute(*args: Any, **kwargs: Any) -> None:
    raise Pair06V8NoOffloadBaselineExecutionAuthorizationRequestHold(NEXT_GATE)
