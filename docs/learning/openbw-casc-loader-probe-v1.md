...
The War College transformer independently implements the narrow loader against
CascLib's public API.
...
with a CascLib-backed loader and links `OpenBWData` to an externally built
CascLib shared library.
...
4. applies the already reviewed BWAPI GCC-13 `<cstdint>` patch;
5. applies the exact-source CASC loader transformer;
6. configures OpenBW/BWAPI headlessly;
7. builds `BWAPILauncher`;
8. verifies `libOpenBWData.so` depends on `libcasc.so`;
9. never executes the launcher or opens game data.
