"""Consume exactly one V2R13 pair-09 candidate launch slot.
...
MARKER_NAME = "pair-09-candidate-attempt-v1.json"
CLAIM_CONFIRMATION = (
    "VOID_ABADDON_GENERATION2_V2R13_RESERVE_PAIR09_CANDIDATE_ATTEMPT"
)
SCHEMA = (
    "void.abaddon.generation2."
    "v2r13-pair09-candidate-attempt-consumption.v1"
)
MAX_MARKER_BYTES = 2048
MAX_IO_CALLS = 64


class V2R13Pair09CandidateAttemptHold(RuntimeError):
...
def _require(condition: bool, code: str, *, consumed: bool = False) -> None:
    if not condition:
        raise V2R13Pair09CandidateAttemptHold(
            code,
...
def _validate_request_review() -> dict[str, Any]:
    ...
    _require(
        reviewed.get("pair09_candidate_authorization_request_reviewed") is True,
        "PAIR09_CANDIDATE_ATTEMPT_REQUEST_NOT_REVIEWED",
    )
    _require(
        reviewed.get("proposal_only_not_authorization") is True,
        "PAIR09_CANDIDATE_ATTEMPT_REQUEST_AUTHORITY_DRIFT",
    )
    _require(
        reviewed.get("pair_slot") == 9
        and reviewed.get("arm") == "candidate"
        and reviewed.get("held_out") is False,
        "PAIR09_CANDIDATE_ATTEMPT_SCOPE_DRIFT",
    )
    _require(
        reviewed.get("maximum_candidate_attempts") == 1
        and reviewed.get("maximum_automatic_retries") == 0,
        "PAIR09_CANDIDATE_ATTEMPT_CARDINALITY_DRIFT",
    )
    _require(
        reviewed.get("pair09_candidate_specific_authorization_accepted") is False
        and reviewed.get("pair09_candidate_execution_authorized") is False
        and reviewed.get("pair09_candidate_execution_performed") is False,
        "PAIR09_CANDIDATE_ATTEMPT_PREMATURE_AUTHORITY",
    )
    _require(
        reviewed.get("next_gate")
        == "V2R13_PAIR09_CANDIDATE_EXECUTION_AUTHORIZATION_REQUIRED",
        "PAIR09_CANDIDATE_ATTEMPT_FRONTIER_DRIFT",
    )
    return reviewed
...
def _directory(
    ...
    *,
    ...
    _require(
        ...
        "PAIR09_CANDIDATE_ATTEMPT_DIRECTORY_HOLD",
        consumed=consumed,
    )
...
def consume_pair09_candidate_attempt(
    *,
    directory_fd: int,
    ...
    """Create and sync the fixed one-use pair-09 candidate marker."""

    _require(
        type(confirm) is str and confirm == CLAIM_CONFIRMATION,
        "PAIR09_CANDIDATE_ATTEMPT_CONFIRMATION_REQUIRED",
    )
    _validate_request_review()

    try:
        validated = request.validate_pair09_candidate_authorization_request(
            request_bytes
        )
    except request.V2R13Pair09CandidateAuthorizationRequestHold:
        raise V2R13Pair09CandidateAttemptHold(
            "PAIR09_CANDIDATE_ATTEMPT_REQUEST_INVALID"
        ) from None
    ...
    _require(
        ...
        "PAIR09_CANDIDATE_ATTEMPT_REQUEST_DIGEST_INVALID",
    )
    _require(
        ...
        "PAIR09_CANDIDATE_ATTEMPT_SOURCE_IDENTITY_INVALID",
    )
    _require(
        type(directory_fd) is int and directory_fd >= 0,
        "PAIR09_CANDIDATE_ATTEMPT_FD_INVALID",
    )
    _require(
        ...
        "PAIR09_CANDIDATE_ATTEMPT_DIRECTORY_IDENTITY_INVALID",
    )
    _require(
        os.name == "posix"
        ...
        "PAIR09_CANDIDATE_ATTEMPT_PLATFORM_UNSUPPORTED",
    )
    ...
    _require(
        ...
        "PAIR09_CANDIDATE_ATTEMPT_MARKER_BOUND",
    )
    ...
    try:
        ...
        try:
            ...
            raise V2R13Pair09CandidateAttemptHold(
                "PAIR09_CANDIDATE_ATTEMPT_ALREADY_EXISTS",
                marker_may_exist=True,
            ) from None
        ...
        for _ in range(MAX_IO_CALLS):
            ...
            _require(
                ...
                "PAIR09_CANDIDATE_ATTEMPT_WRITE_HOLD",
                consumed=True,
            )
            offset += written
        _require(
            offset == len(raw),
            "PAIR09_CANDIDATE_ATTEMPT_WRITE_BOUND",
            consumed=True,
        )
        ...
        _require(
            ...
            "PAIR09_CANDIDATE_ATTEMPT_FILE_HOLD",
            consumed=True,
        )
        ...
        for _ in range(MAX_IO_CALLS):
            ...
            _require(
                bool(chunk),
                "PAIR09_CANDIDATE_ATTEMPT_SHORT_READ",
                consumed=True,
            )
        ...
        _require(
            ...
            "PAIR09_CANDIDATE_ATTEMPT_READBACK_HOLD",
            consumed=True,
        )
        ...
        _require(
            ...
            "PAIR09_CANDIDATE_ATTEMPT_FILE_CHANGED",
            consumed=True,
        )
        _directory(
            owned_directory,
    ...
    except V2R13Pair09CandidateAttemptHold:
        failed = True
        raise
    except OSError:
        failed = True
        raise V2R13Pair09CandidateAttemptHold(
            "PAIR09_CANDIDATE_ATTEMPT_IO_HOLD",
        ...
        if close_failed and not failed:
            raise V2R13Pair09CandidateAttemptHold(
                "PAIR09_CANDIDATE_ATTEMPT_CLOSE_HOLD",
                marker_may_exist=marker_may_exist,
            )


def pair09_candidate_attempt_guard_contract() -> dict[str, Any]:
    reviewed = _validate_request_review()
    return {
        "schema": SCHEMA,
        "pair09_candidate_attempt_guard_implemented": True,
        "pair09_candidate_attempt_guard_reviewed": False,
        "pair_slot": 9,
        "arm": "candidate",
        "held_out": False,
        "marker_name": MARKER_NAME,
        "create_only_marker": True,
        ...
        "next_gate": "V2R13_PAIR09_CANDIDATE_ATTEMPT_GUARD_SOURCE_BINDING_REVIEW_REQUIRED",
        "next_change_class": (
            "source_only_v2r13_pair09_candidate_attempt_guard_source_binding_review"
        ),
    }
